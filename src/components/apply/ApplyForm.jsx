...
export default function ApplyForm({ state, handleInputChange, handleApplicationSubmit }) {
    const { formData, status, isBlocked } = state;
    const isSubmitting = status === 'submitting';
    const [showRequirements, setShowRequirements] = useState(false);

    return (
        <Fieldset.Root as="form" onSubmit={handleApplicationSubmit} bg="whiteAlpha.900" backdropFilter="blur(20px)" p={{ base: '8', lg: '12' }} rounded="3xl" boxShadow="2xl" border="1px solid" borderColor="whiteAlpha.300">
            <Stack>
            ...
            <Box my="8">
                <Collapsible.Root open={showRequirements} onOpenChange={setShowRequirements}>
                    <Collapsible.Trigger asChild>
                        <Button w="full" display="flex" alignItems="center" justifyContent="space-between" bgGradient="linear(to-r, orange.50, orange.100)" p="4" rounded="xl" border="1px solid" borderColor="orange.200" _hover={{ bgGradient: 'linear(to-r, orange.100, #f8e5d3)' }} transition="all 0.3s">
            ...
            <Button type="submit" disabled={isSubmitting || isBlocked} w="full" bgGradient="linear(to-r, orange.500, orange.600)" color="white" mt="6" _hover={{ bgGradient: 'linear(to-r, orange.600, orange.700)' }} _disabled={{ opacity: '0.5', cursor: 'not-allowed' }}>
